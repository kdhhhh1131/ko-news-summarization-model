...
본 프로젝트는 **'1.5B 규모의 초경량 언어 모델(sLLM)이 방대한 뉴스 텍스트에서 정보 압축 및 팩트 추출 역량을 어디까지 발휘할 수 있는지 검증'**하는 데 목적이 있습니다.

이를 위해 긴 뉴스 기사를 입력하면 글의 흐름 속에서 인과관계나 군더더기 서술은 과감히 생략하고, 가장 중요한 '팩트(Fact)'와 '수치'만을 남겨 명료하게 3줄로 요약해 내도록 파인튜닝을 진행했습니다.

작은 체급에도 불구하고 한국어 문해력이 뛰어난 Qwen2.5-1.5B를 베이스 모델로 선정하여 정보의 압축률을 극대화했으며, bnb-4bit 양자화 기법을 적용해 무료 Colab 환경(T4 GPU)에서도 제한 없이 원활한 실험과 추론이 가능하도록 설계했습니다.
<br/>
...
### 1. 모델 및 데이터 선택 근거
* **모델 선택 (Qwen2.5-1.5B)**: 1.5B 초경량 사이즈임에도 불구하고 한국어 토큰 효율과 문해력이 우수합니다. 특히 `bnb-4bit` 양자화 모델을 활용하여 무료 Colab 환경(T4 GPU)에서도 VRAM 부족 현상 없이 원활하고 빠른 학습이 가능하도록 설계했습니다. 
* **데이터셋 선택**: 네이버 뉴스 본문과 전문가가 요약한 요약문이 쌍(Pair)으로 이루어져 있어, 모델에게 '불필요한 문장 쳐내기'와 '핵심 압축'의 패턴을 가르치기에 가장 이상적인 데이터라고 판단했습니다.
...
### 1. 환경 구성 및 실행 방법 (Colab 권장)

본 모델은 4-bit 양자화(Quantization)된 베이스 모델에 파인튜닝된 LoRA 가중치를 병합(Merge)하여 추론하는 방식으로 동작합니다. 제한된 리소스를 효율적으로 사용하도록 설계되어, **Google Colab의 무료 T4 GPU 환경**에서 원활하게 실행할 수 있습니다.

#### 🚀 빠른 실행 가이드
1. 첨부된 `ko-news-summarization-model.ipynb` 파일을 Google Colab에서 엽니다.
2. 런타임 유형을 **T4 GPU**로 설정합니다. (`런타임` > `런타임 유형 변경`)
3. 노트북 내의 텍스트 변수(`article`)에 요약하고자 하는 뉴스 기사 본문을 붙여넣습니다.
4. 위에서부터 순서대로 셀을 실행하면 베이스모델과 파인튜닝 모델의 3줄 요약 결과를 비교하며 확인할 수 있습니다.
```bash
...
* **LoRA 어댑터 병합 (`PeftModel.from_pretrained`)**
  본 저장소의 모델 파일은 전체 가중치가 아닌 학습된 파라미터만 담긴 **LoRA 어댑터**입니다. 따라서 4-bit로 양자화된 가벼운 베이스 모델을 먼저 메모리에 올린 뒤, 그 위에 어댑터를 덧씌우는(Merge) 방식을 사용하여 VRAM 사용량을 절약합니다.
