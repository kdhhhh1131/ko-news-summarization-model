...
* **프롬프트 템플릿 유지 (`apply_chat_template`)**
  모델이 학습될 때 사용했던 구조(System Prompt, User Prompt)를 추론 시에도 완벽하게 일치시켜 주는 함수입니다. 학습 환경과 동일한 템플릿을 제공해야 모델이 지시사항(3줄 불렛 포인트 요약)을 정확하게 따릅니다.

* **텍스트 생성 파라미터 제어 (`model.generate`)**
  * `temperature=0.3` & `top_p=0.9`: 모델의 창의성을 낮추어, 환각(Hallucination) 현상 없이 **기사 본문의 팩트에 기반한 건조하고 일관된 요약**을 생성하도록 유도합니다.
  * `repetition_penalty=1.2`: 모델이 텍스트를 생성할 때 동일한 단어나 문장을 무의미하게 반복하는 것을 방지합니다.

* **순수 생성 결과 파싱 (`outputs[0][input_length:]`)**
  언어 모델은 기본적으로 입력된 질문(프롬프트)의 꼬리를 물고 텍스트를 생성합니다. 깔끔한 출력 결과를 얻기 위해, 입력된 프롬프트의 토큰 길이를 계산하여 **모델이 새롭게 만들어낸 요약 텍스트 영역만 정확히 잘라내는** 후처리 로직입니다.
