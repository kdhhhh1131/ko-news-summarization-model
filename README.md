...
### 1. 모델 및 데이터 선택 근거
* **모델 선택 (Qwen2.5-1.5B)**: 1.5B 경량 사이즈임에도 불구하고 한국어 토큰 효율과 문해력이 매우 뛰어납니다. 특히 `bnb-4bit` 양자화 모델을 활용하여 무료 Colab 환경(T4 GPU)에서도 VRAM 부족 현상 없이 원활하고 빠른 학습이 가능하도록 설계했습니다. 
